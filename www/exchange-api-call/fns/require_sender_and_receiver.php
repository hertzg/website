...
function require_sender_and_receiver ($mysqli, $permission,
    ...
    if ($sender_username === '') {
        ...
        ErrorJson\badRequest('"ENTER_SENDER_USERNAME"');
    }

    include_once "$fnsDir/Username/isValid.php";
    if (!Username\isValid($sender_username)) {
        include_once "$fnsDir/ErrorJson/badRequest.php";
        ErrorJson\badRequest('"INVALID_SENDER_USERNAME"');
    }

    if ($receiver_username === '') {
        include_once "$fnsDir/ErrorJson/badRequest.php";
        ErrorJson\badRequest('"ENTER_RECEIVER_USERNAME"');
    }

    if (!Username\isValid($receiver_username)) {
        include_once "$fnsDir/ErrorJson/badRequest.php";
        ErrorJson\badRequest('"INVALID_RECEIVER_USERNAME"');
    }
    ...
    if (!$receiver_user) {
        include_once "$fnsDir/ErrorJson/badRequest.php";
        ErrorJson\badRequest('"RECEIVER_NOT_FOUND"');
    }
    ...
    if (!$connection[$permission]) {
        include_once "$fnsDir/ErrorJson/badRequest.php";
        ErrorJson\badRequest('"RECEIVER_NOT_RECEIVING"');
    }

...
function edit ($mysqli, $apiKey, $name, $expire_time,
    ...
    $can_write_wallets, &$changed) {

    if ($apiKey->name === $name) {
        if (($apiKey->expire_time === null && $expire_time === null) ||
            (int)$apiKey->expire_time === $expire_time) {

            if ((bool)$apiKey->can_read_bar_charts === $can_read_bar_charts &&
                (bool)$apiKey->can_read_bookmarks === $can_read_bookmarks &&
                (bool)$apiKey->can_read_calculations === $can_read_calculations &&
                ...
                (bool)$apiKey->can_write_calculations === $can_write_calculations &&
    ...
    \ApiKeys\edit($mysqli, $id, $name, $expire_time,
        ...
        $can_read_notes, $can_read_notifications, $can_read_places,
        $can_read_schedules, $can_read_tasks, $can_read_wallets,
        $can_write_bar_charts, $can_write_bookmarks,
        $can_write_calculations, $can_write_channels,
    ...
    include_once "$fnsDir/Calculations/editApiKey.php";
    \Calculations\editApiKey($mysqli, $id, $name);

...
function add ($mysqli, $id_users, $id_tasks, $tag_names,
    ...
    $sql = 'insert into task_tags'
        .' (id_users, id_tasks, tag_name, text, title, deadline_time, tags,'
        .' tags_json, top_priority, insert_time, update_time) values';
    foreach ($tag_names as $i => $tag_name) {
        if ($i) $sql .= ', ';
        $tag_name = $mysqli->real_escape_string($tag_name);
        $sql .= "($id_users, $id_tasks,"
            ." '$tag_name', '$text', '$title', $deadline_time, '$tags',"
